...
//ok so what i did now is just put the other project in the build path
//idk if it's better to just re-include all the classes ? 
// we wanted to include initializing roads and towns here too ?
public class ClientGame {
	...
	public void chooseBoot(BootColor c) {
		if (thisPlayer.getBoot() == BootColor.UNDECIDED) {
			//send message of move to server
		}
		return;
	}
	...
	public boolean drawRandomCounter() {
		//send message
		return true;
	}
	
	//same for draw counter 
	public boolean drawCounter(Counter c) {
		//send message
		return true;
	}
	
	public boolean placeCounter(Counter c, Road r) {
	...
	public boolean travelOnRoad(Road r, ArrayList<TravelCard> cardsUsed) {
		//false if road is not connected to player location
		if (thisPlayer.getLocation() != r.getSourceTown() && thisPlayer.getLocation() != r.getDestinationTown()) {
			return false;
		}
		...
		if(r.getRegionKind() == RegionKind.RIVER) {
			...
			if(cardsUsed.size() == 1 && thisPlayer.getLocation() == r.getSourceTown()) {
				//send message to server
				return true;
			}
			//true if two cards and coming from destination town
			else if(cardsUsed.size() == 2 && thisPlayer.getLocation() == r.getDestinationTown()) {
				//send message to server
				return true;
			}
	...
	//also same
	public boolean chosenCounterToKeep(Counter c) {
		//send message
		return true;
	}
