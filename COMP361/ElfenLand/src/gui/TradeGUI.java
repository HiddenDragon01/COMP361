...
public class TradeGUI extends ActionGUI
{
	...
	public TradeGUI(GameGUI gameGUI) {
		super(gameGUI, "Propose a trade", "is contemplating a new trade");
	}
	...
	protected void sendSelection() {
		...
		Trade t = new Trade();
		t.setP1(gameGUI.game.getCurrentPlayer());
		if(!chosenCards.isEmpty() || !chosenCounters.isEmpty()) {
			t.setOfferedCards1(chosenCards);
			t.setOfferedCounters1(chosenCounters);
			t.setP1locked(true);
		}
		t.setRequestedItems1(requestDescription);
		
		gameGUI.game.createInitialTrade(t);
	...
	protected boolean checkAllSelected() {
		...
		{
			gameGUI.displayMessage("ERROR!\nYou must describe your request. Try again.");
		}
