...
public enum TownName implements Serializable {
	...
	private void assignGoldValue(int value) {
	...
	public static void randomizeGoldValues() {
		...
		for(TownName t : TownName.values()) {
			if(t != ELVENHOLD) {
				t.assignGoldValue(possibleValues.remove(0).intValue());
				
				//COMMAND send town, gold value
				Server.serverChannel.sendTownValue(t, t.getGoldValue());
			}
		}
